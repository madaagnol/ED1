...
void InserirArvBB(ABBusca **ab, int valor)
{
  if ((*ab) == NULL) { 
    (*ab) = CriaNovoNo(valor);
  } else {
    if ((*ab)->info <= valor)
     InserirArvBB(&((*ab)->dir),valor);
    else
      InserirArvBB(&((*ab)->esq),valor);		   
  }
}
...
void RetiraArvBB(ABBusca **ab, int valor)
{
  ABBusca *aux;
  ...
  if ((*ab) != NULL){  
    if ((*ab)->info != valor) {
      if ((*ab)->info < valor) { 
	RetiraArvBB(&((*ab)->dir),valor);
      } else {
	RetiraArvBB(&((*ab)->esq),valor);
      }
    } else { /* (*ab)->info == valor */
